"""
Train eSEN Student via Response Distillation (energy + force matching)

Usage:
    python training/train_esen_student.py --config configs/ethanol_esen.yaml
    python training/train_esen_student.py --config configs/ethanol_esen.yaml --use_original_data
    python training/train_esen_student.py --config configs/ethanol_esen.yaml --no-wandb
"""
...
def main():
    ...
    if use_wandb:
        try:
            ...
            run_name = args.wandb_name or f"esen_L{sc['lmax']}_C{sc['sphere_channels']}_{config['data']['molecule']}_{data_source}"
            wandb.init(project=args.wandb_project, name=run_name,
                       config={"phase": "student", "model": "eSEN", "data_source": data_source, **config},
                       tags=["student", "esen", config["data"]["molecule"], data_source])
    ...
    print(f"eSEN Student: {n_params:,} parameters")
    print(f"  num_layers={sc['num_layers']}, lmax={sc['lmax']}, mmax={sc['mmax']}")
    print(f"  sphere_channels={sc['sphere_channels']}, hidden_channels={sc['hidden_channels']}")
    if use_wandb:
        import wandb
    ...
    scheduler = CosineAnnealingLR(optimizer,
        T_max=sc["max_epochs"] - sc.get("warmup_epochs", 5),
        eta_min=sc["min_lr"])

    warmup_epochs = sc.get("warmup_epochs", 5)
    ...
    print(f"\n{'='*60}\nStarting eSEN student training")
